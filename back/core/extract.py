...
class APKExtractor:
    ...
    def _get_strings(self) -> Dict:
        ...
        for s_obj in self.analysis.get_strings():
            ...
            # Base64 candidates
            if len(s) > 30 and re.match(r'^[A-Za-z0-9+/=]+$', s) and s.endswith('='):
        ...
        try:
            with zipfile.ZipFile(self.apk_path, 'r') as z:
                ...
                return {
                    ...
                    "resources": [f for f in all_files if f.startswith('res/')],
...
def main():
    ...
    if "error" in data:
        ...
        print(f"Permissions: {len(data['permissions']['requested'])}")
        ...
        print(f"\nNetwork ops: {len(data['bytecode_references']['network_operations'])}")
        ...
        print(f"\nURLs found: {len(data['strings']['urls'])}")
        print(f"IPs found: {len(data['strings']['ip_addresses'])}")
        print("="*60)


if __name__ == "__main__":
    main()
def extract_apk_data(apk_path: str):
    extractor = APKExtractor(apk_path)
